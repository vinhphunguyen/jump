...
function main()
	...
	fOffset  = grid.dx/ppc
	fOffsetR = grid.dx/2
	coords1  = buildParticleForRing([outerDiameter/2; outerDiameter/2], 0.5*innerDiameter, 0.5*outerDiameter, fOffset)
	...
	output2  = OvitoOutput(interval,"cellular/",["pstrain", "vonMises"])
	fix      = EmptyFix()
	...
	plotParticles(output2,solids,[grid.lx, grid.ly],[grid.nodeCountX, grid.nodeCountY],0)
