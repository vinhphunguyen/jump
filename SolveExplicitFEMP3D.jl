...
######################################################################
function solve_explicit_dynamics_femp_3D(grid,solids,basis,body,alg::TLFEM_MUSL,output,fixes,Tf,dtime)
    t       = 0.
    counter = 0
  ...
  while t < Tf
    ...
    @inbounds for i = 1:grid.nodeCount
	  nodalMass[i]      = 0.
	  nodalMomentum0[i] =  @SVector [0., 0., 0.]
	  nodalMomentum2[i] =  @SVector [0., 0., 0.]
	  nodalForce[i]     =  @SVector [0., 0., 0.]
    end
        ...
        # loop over solid elements, not solid nodes
	  	@inbounds for ip = 1:solid.parCount
			...
			dstrain      = 0.5 * (vel_grad + vel_grad' + vel_grad * vel_grad') - strain[ip] 
		   	...
		   	#println(strain[ip])
	   	     #@timeit "3" update_stress!(stress[ip],mat,strain[ip],F[ip],J,ip)
	   	    update_stress!(stress[ip],mat,strain[ip],dstrain,F[ip],J,ip,dtime)
        ...
        # loop over  elements of the surface tag 'force'
	  	@inbounds for ip in surf_elems_ids
			...
			for ip=1:4                     
				...
				for i = 1:length(elemNodes)
				   in  = elemNodes[i]; # index of node 'i'
		           ftrac[in][1] -= normals_surface[1,ip]*funcs_surface[i,ip]*ww*100*sin(π*t/Tf)
		           ftrac[in][2] -= normals_surface[2,ip]*funcs_surface[i,ip]*ww*100*sin(π*t/Tf)
		           ftrac[in][3] -= normals_surface[3,ip]*funcs_surface[i,ip]*ww*100*sin(π*t/Tf)
		        end
	        end
	   end
	end
