...
# Output in folder "twodisks-femp-tl/", with vtu files and energies.txt
...
Fem.move(solid2, SVector{2,Float64}([0.2 + 0.6, 0.2 + 0.6]))
...
bodyforce = ConstantBodyForce2D([0.0, 0.0])
algo1     = TLFEM(0.0, 0.99)
...
#reset_timer!
solve_explicit_dynamics_femp_2D(
    grid,
    solids,
    mats,
    basis,
...
# plotting energies
pyFig_RealTime =
    PyPlot.figure("MPM 2Disk FinalPlot", figsize = (8 / 2.54, 4 / 2.54))
PyPlot.clf()
pyPlot01 = PyPlot.gca()
